...
<?php //$header = get_field('blog_header', 'option'); ?>

<?php
    $ParentTitle = wp_title('@',FALSE);
	 $ChildTitle = substr($ParentTitle, strpos($ParentTitle, '@') + 1);
	  $GrandChildTitle = substr($ChildTitle, strpos($ChildTitle, '@') + 1);
    $last_word = $GrandChildTitle;
	 $url = explode(' ', $_SERVER['REQUEST_URI']);
	 $urlstring = rtrim($url[0],'/');
	 $secondlastWord = substr($urlstring, strpos($urlstring, '/') + 1);
	 $lastWord = substr($secondlastWord, strpos($secondlastWord, '/') + 1);
	 $lastWord = array_shift(explode('/', $lastWord));	 
?>
...
<div class="blog-section-four mt-160 mb-150 lg-mt-100 lg-mb-120">
    <div class="container mb-40">
        <div class="row">
            <div class="col-xl-10">
                <!--Animated Block-->
                <h2>New & Events Category - <?= $last_word; ?></h2>
            </div>
        </div>
    </div>
    <div class="container">
                ...
                <div class="blog-meta-wrapper">
                    ...
                    <article class="blog-meta-four">
                        <div class="img-meta position-relative">
                            ...
                            <a href="#" class="tag"><?= $term_string;?></a>
                        </div>
